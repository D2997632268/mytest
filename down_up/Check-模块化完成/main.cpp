...
//    cout<<"输出="<<(pow(slope,2)+1)<<endl;
    ...
    for(int j=0; j<gray->width-start_x; j+=5)
    {
        ...
        {
            data[j] = *((uchar *)gray->imageData+j*gray->widthStep+i)+0;
//            cout<<data[j]<<endl;
        }
...
int plotPoint(IplImage *image, IplImage *gray ,int pixel4XYX, int pixel4XYY, double k_horizontal, double k_v, int vmidX1, int vmidY1, int hmidX1, int hmidY1, int *X, int *Y)
{
...
int accurateSearchPixel4XY(IplImage *image, IplImage *gray ,double k_horizontal, double k_v, int sumx, int sumyy, int *X, int *Y)
{
...
//IplImage *image, IplImage *gray ,int pixel4XYX, int pixel4XYY, double k_horizontal, double k_v
    int result = plotPoint(image, gray,pixel4XY[0], pixel4XY[1], k_horizontal,  k_v, vmidX1, vmidY1, hmidX1, hmidY1, X, Y);
//    if(len(result)=1)
//    {
//        return 0;
//    }else{
...
//            printf("X[0]=%d, Y[0]=%d\n", X[0], Y[0]);
            ...
            if(re)
            {
                int result = accurateSearchPixel4XY(image, gray, k_horizontal, k_v, sumx, sumyy, X, Y);
                return result;
            }
...
int main()
{
    int finder_scan(IplImage* image, IplImage* gray/*, int* decode_result*/);
...
//    Mat src = imread("1-20.jpg");
    cout<<"\n-------start------\n"<<endl;
    //读取图像
    IplImage* image = cvLoadImage("pic/190.jpg");//image/c900  image_c/c11  57 o2  a/c48  image_c3/c75
//    Mat src = cvarrToMat(image);
    ...
    int result = finder_scan(image, gray/*, decode_result*/);
