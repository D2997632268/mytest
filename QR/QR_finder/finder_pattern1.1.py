...
def FloodFill2(image,x,y,book):
    ...
    # print('y=',y)
	# for i in x:
	# 	cv2.circle(img, (i,y),1,(0,0,255),1)
...
def perspectivate_setup(rect,w,h):
    ...
    # a = w*(x2*y3-x3*y2 + (x3-x2)*y1 + x1*(y2-y3))
            ...
            for xW in range(0, len(Xwidth) - 4):
                ...
                if ok == 1:  # 水平扫描结束
                    ...
                    if XX != [] and y != [] :
                        ...
                        cv2.floodFill(imgCopy2, mask3, (Row[0][0] - 1, Col[0]),
                                      (255, 0, 0), cv2.FLOODFILL_FIXED_RANGE)
                        # pass
