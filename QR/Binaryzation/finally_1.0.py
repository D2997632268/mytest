	...
	for i in range(x,length-10,10):
		...
		if value==0:
			...
			if number>=2:
				...
				# 		print('end error!!')
				# else:
	# 				print('decodeNUM[number-1] != head_39', decodeNUM[number-1], head_39)
	# print('loop_finally',i)
	return 0
...
def LoopDecode():
	start = time()
	while 1:
		global head_39, data_test, fx, fxx, edgeWidth, edgeWidth_1, outer, maopaotab, BS, COL, boolCOOL,loop_number,yy,fx_size,wav,baoluoupData,baoluodownData,LEN,edge,z_result,length,d
		head_39 = 0
		data_test = []
		fx = [] #一阶差分
		fxx = [] #二阶差分
		...
		yy = y.copy()
		LEN = len(y)
		# cv2.line(image, (0,line), (n,line), (255,0,0),1)
