...
def end(rightcp, pos, maxtimes):
	...
	# 		return 1
	# if tem==2:
	# 	print('i=',i, d[pos+1], d[pos+2] )
	# 	if d[pos+1]>=i or d[pos+2]>=i:
	# 		return 1
	if tem>=3:
		# print('i=',i, d[pos+1], d[pos+2] , d[pos+3])
		if d[pos+1]>=i or d[pos+2]>=i or d[pos+3]>=i:
			return 1
...
def LoopDecode():
	...
	while 1:
		global preprocess_time, head_39, data_test, fx, fxx, edgeWidth, edgeWidth_1, outer, maopaotab, BS, COL,COL2, boolCOOL,loop_number,yy,fx_size,wav,baoluoupData,baoluodownData,LEN,edge,z_result,length,d
		head_39 = 0
		data_test = []
		fx = [] #一阶差分
		fxx = [] #二阶差分
...
while ret:
	...
	if C%timeF==0 :
		...
		if result:
			...
			success += 1
			cv2.putText(image, str(decodeNUM), (10,20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2, cv2.LINE_AA)
			cv2.imwrite('success/'+str(success)+'.bmp',image)
			print('preprocess_time + decode_time=',(preprocess_time-start1),'+',(end_time1-preprocess_time))
			print('time:{:.2f}'.format(end_time1-start1))
			print('DECODE',decodeNUM)
		else:
			fail += 1
			cv2.imwrite('failture/'+str(fail)+'.bmp',image)
