...
extern void binaryzation(int *y, int * X_position, int *X_width , int *num, int *length);
...
//        printf("i=%d\n", i);
        ...
        int length = height;
//        printf("in\n");
        binaryzation(/*image,*/ data, y_col, Ywidth, &LEN_y, &length);
        if(LEN_y<8)
        {
            continue;
        }
...
int accurateSearchPixel4XY(unsigned char* pic ,double k_horizontal, double k_v, int sumx, int sumyy, int *X, int *Y)
{
    ...
    if (temp1>300000 || temp1<0 || temp2>300000 || temp2<0)
    {
        return 0;
    }
...
//                    printf("从下往上找边缘");
    ...
    if(BOOL)
    {
        ...
        if (temp2>300000 || temp2<0 || temp1>300000 || temp1<0 )
        {
            return 0;
        }
...
//                printf("%d\t", data[j]);
        ...
        binaryzation(data, Xposition, Xwidth, &LEN, &length);
//        printf("length=%d\n",LEN);
//        for(int i=0; i<LEN; i++)
//        {
...
void main()
{
    ...
    {
        printf("\n-------start------\n");
        char buffer[32]="decode_picture2\\frame+";//picture_319  new_picture  decode_picture
        char cacahe[6];
        fra += 1;
