...
//                    printf("ax[0]-ay_width[0]=%d, y=%d\n", ax[0]-pd[0], y);
                    ...
                    key=1;
                    memset(book, 0, sizeof(book));
                    int flood_a = flood_fill_seed(pic, ax[1]+pd[2]/2, y, nextXd, nextYd, book);
//                    printf("flood_a=%d\n", flood_a);
                    //*******找到条码最下边的点*********//
                    if(flood_a)
                    {
...
//                           printf("dis-y<sumx!!\n");
                           ...
                           key=1;
                           int a=0;
                           a = flood_fill_seed(pic, ax[1]+pd[2]/2, y, nextXu, nextYu, book);
//                           printf("pos[0]=%d, pos[1]=%d\n", pos[0], pos[1]);
                           if(a)
                           {
        ...
        if(x_num>=2)
        {
            printf("324-test--pre_X[2]=%d, ax[2]=%d\n", pre_X[2], ax[2]);
            printf("324-test--pre_y=%d, y=%d\n", pre_y, y);
            int temp_x = (pre_X[2]+pos[0])/2;
            int temp_y = (ax[2]+poss_down)/2;
            int con=0;//pos[0] poss_down
            printf("324---pic=%d\n",pic[(pre_y-1)*width+pre_X[2]]+0);
            if(((temp_y-1)*width+temp_x)>limit)
            {
                return 0;
            }
            if(pic[(temp_y-1)*width+temp_x]+0<pixelMax)
            {
                while(con<(pre_X[2]-pre_X[1]) && pic[(temp_y-1)*width+temp_x]+0<pixelMax)
                {
                    con += 1;
                    temp_x += 1;
                    if(((temp_y-1)*width+temp_x)>limit)
            ...
            }else if(pic[(temp_y-1)*width+temp_x]+0>pixelMin)
            {
                while(con<(pre_X[2]-pre_X[1]) && pic[(temp_y-1)*width+temp_x]+0>pixelMin)
                {
                    con += 1;
                    temp_x -= 1;
                    if(((temp_y-1)*width+temp_x)>limit)
            ...
            printf("con=%d\n", con);
            //三点拟合直线
            int xx[3]={pre_X[2], temp_x, ax[2]};
            int yy[3]={pre_y, temp_y, y};
            float KB[2] = {0.0, 0.0};
            Least_squares(xx, yy, 3, KB);
            printf("KB[0]=%f, KB[1]=%f\n", KB[0], KB[1]);
            k_horizontal = KB[0];
            slope = -k_horizontal;
            b = KB[1];
            printf("324-test--temp_x=%d, temp_y=%d\n", temp_x, temp_y);
//            printf("x_num>=2\n");
//            if(ax[1]-pre_X[1]==0)
//            {
//                slope=1;
...
//            }
            ...
            printf("324test---pos[0]=%d, poss_down=%d\n", pos[0], poss_down);
...
//                printf("2a=%d, 2b=%d\n", aa,bb );
            ...
            pos[1]=0;
            key=1;
            flood_fill_seed(pic, ROW, ay[1]+ay_width[2]/2, nextXu, nextYu, book);
            printf("pos[0]=%d, pos[1]=%d\n", pos[0], pos[1]);
...
//                printf("%d, %d, %d\n", pos_x, ROW, sumy);
                if(pos_x-ROW<sumy)
                {
                    ...
                    pos[1]=0;
                    key=1;
                    flood_fill_seed(pic, ROW, ay[4]+ay_width[4]/2, nextXu, nextYu, book);
//                    printf("pos[0]=%d, pos[1]=%d\n", pos[0], pos[1]);
                    pos_x = pos[0] + sumx/7;
                    pos_xi = 1;
                }
...
//            printf("---test_vertical---out\n");
        ...
        //                printf("------k_hor*k_V-->%f*%f=%f\n", k_horizontal,k_v, k_horizontal*k_v);
            ...
            //test//
            int temp_x2 = (ROW+ROW2)/2;
            int temp_y2 = (ay[2]+ay2[2])/2-1;
            int con2=0;//pos[0] poss_down
            printf("垂直测试---temp_x2=%d, temp_y2=%d\n", temp_x2, temp_y2);

            if(((temp_y2-1)*width+temp_x2)>limit)
            {
                return 0;
            }
            printf("324---pic=%d\n",pic[(ay2[2]-1)*width+ROW2]+0);
            if(pic[(temp_y2-1)*width+temp_x2]+0<pixelMax)
            {
                while(con2<(ay2[2]-ay2[1]) && pic[(temp_y2-1)*width+temp_x2]+0<pixelMax)
                {
                    con2 += 1;
                    temp_y2 += 1;
                    if(((temp_y2-1)*width+temp_x2)>limit)
            ...
            }else if(pic[(temp_y2-1)*width+temp_x2]+0>pixelMin){
                while(con2<(ay2[2]-ay2[1]) && pic[(temp_y2-1)*width+temp_x2]+0>pixelMin)
                {
                    con2 += 1;
                    temp_y2 -= 1;
                    if(((temp_y2-1)*width+temp_x2)>limit)
...
//            //三点拟合直线
//            printf("垂直测试1---temp_x2=%d, temp_y2=%d\n", ROW, ay[2]);
//            printf("垂直测试1.5---temp_x2=%d, temp_y2=%d\n", temp_x2, temp_y2);
//            printf("垂直测试2---temp_x2=%d, temp_y2=%d\n", ROW2, ay2[2]);
            int xx2[3]={ROW, temp_x2, ROW2};
            int yy2[3]={ay[2], temp_y2, ay2[2]};
            float KB2[2] = {0.0, 0.0};
            Least_squares(xx2, yy2, 3, KB2);
            printf("KB2[0]=%f, KB2[1]=%f\n", KB2[0], KB2[1]);
            k_v = KB2[0];
            b_v = -k_horizontal;
            b = KB2[1];
