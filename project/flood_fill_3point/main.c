...
//                           printf("pos[0]=%d, pos[1]=%d\n", pos[0], pos[1]);
        ...
        if(x_num>=2)
        {
            ...
            if(((temp_y-1)*width+temp_x+1)>limit)
            {
                return 0;
            }
            while(con<(pre_X[2]-pre_X[1]) && pic[(temp_y-1)*width+temp_x+1]+0<pixelMax)
            {
                con += 1;
                temp_x += 1;
                if(((temp_y-1)*width+temp_x+1)>limit)
...
//            printf("垂直测试2---temp_x2=%d, temp_y2=%d\n", ROW2, ay2[2]);
            ...
            printf("324-test--pre_X[2]=%d, ax[2]=%d\n", ROW, ay[2]);
            printf("324-test--ROW2=%d, ay2[2]=%d\n", ROW2, ay2[2]);
            float KB2[2] = {0.0, 0.0};
