...
int flood_fill_seed2(unsigned char* pic, int x, int y, int *nextX, int *nextY, int *book);
...
//        printf("像素=%d\n", pic[ty*width+tx]);
        ...
        {
            book[ty*width+tx] = 1;
            flood_fill_seed2(pic, tx,  ty, nextX, nextY, book);
        }
//        printf("---------key=%d\n", key);
        if(key==0)
        {
...
//printf("sumy=%d, ROW=%d\n", sumy, i);
            ...
            {
//                printf("前black1[ia]=%d, white1[ia]=%d\n", black1[ia], white1[ia]);
                aa = (int)(ay[ia]+1*pow(-1, ia+1))*width+*(ROW);
                bb = (int)(ay[ia]+1*pow(-1, ia+2))*width+*(ROW);
//                printf("2a=%d, 2b=%d, limit=%d\n", aa,bb ,limit);
                ...
                black1[ia] = pic[aa]+0;
                white1[ia] = pic[bb]+0;
//                printf("后black1[ia]=%d, white1[ia]=%d\n", black1[ia], white1[ia]);
...
//printf("-----flood vertical  book out----\n");
            ...
            int flood_y2[width];
            int flood_num2=0;
            printf("-----flood vertical  flood_fill_seed in-ROW=%d---\n",*ROW);
            printf("x=%d, y=%d\n", *ROW,ay[1]+ay_width[2]/2);
            int flood = flood_fill_seed(pic, *ROW, ay[1]+ay_width[2]/2, nextXu, nextYu, book,flood_x2,flood_y2,&flood_num2);
            printf("-----垂直flood vertical  flood_fill_seed out-flood=%d---\n", flood);
            printf("flood_num2=%d\n", flood_num2);
            if(flood_num2<=10)
            {
                printf("flood_num2<=10--continue!!\n");
                continue;
            }
            if(flood==0)
            {
                printf("flood--continue!!\n");
                continue;
            }
...
//                                   printf("pic=%d\n",pic[(flood_y2[wi]-1)*width+flood_x2[wi]]+0);
                        ...
                        if(pic[(flood_y2[wi]-1)*width+flood_x2[wi]]+0<pixelMin)
                        {
//                                       printf("--in--");
                            flood_y2[wi] += 1;
                        }else{
                            if(((flood_y2[wi]+3-1)*width+flood_x2[wi])>limit || ((flood_y2[wi]+3-1)*width+flood_x2[wi])<0)
                            {
                                return 0;
                            }
                            if(pic[(flood_y2[wi]+3-1)*width+flood_x2[wi]]+0<pixelMin)
                            {
    //                                       printf("--in--");
                                flood_y2[wi] += 3;
                            }else{
...
//        printf("像素=%d\n", pic[ty*width+tx]);
        if(book[ty*width+tx] == 0 && (pixelMax >= pic[ty*width+tx]+0))
        {
            book[ty*width+tx] = 1;
...
//                    printf("i=%d,j=%d, midx=%d, midy=%d\n", i,j, midx, midy);
//                    printf("%d\t",midy);
//                    printf("pic[temp]=%d\t", pic[temp]+0);
//                    printf("threshld=%d\t",(pixelMin+pixelMax)/2);
...
//                    printf("i=%d,j=%d, midx=%d, midy=%d\t", i,j, midx, midy);
//                    printf("pic[temp]=%d\t", pic[temp]+0);
//                    printf("threshld=%d\t",(pixelMin+pixelMax)/2);
                    printf("%d \t",midy);
                    if(pic[temp]+0>(pixelMin+pixelMax)/2)
...
//                       printf("down__dis-y=%d\n", dis-y);
                       ...
                       for(int wi=0; wi<flood_num1; wi++)
                       {
                           ...
                           if(flood_y1[wi]<poss_down)
                           {
                               ...
                               while(con<(ax[2]-ax[1]))
                               {
                                   ...
                                   if(pic[(flood_y1[wi]-1)*width+flood_x1[wi]]+0<pixelMax)
                                   {
                                       flood_x1[wi] += 1;
                                   }else{
                                       if(((flood_y1[wi]-1)*width+flood_x1[wi]+3)>limit || ((flood_y1[wi]-1)*width+flood_x1[wi]+3)<0)
                                       {
                                           return 0;
                                       }
                                       if(pic[(flood_y1[wi]-1)*width+flood_x1[wi]+3]+0<pixelMax)
                                       {
                                           flood_x1[wi] += 3;
                                       }else{
                                           break;
                       ...
                       key=1;
                       int a=0;
                       a = flood_fill_seed2(pic, ax[1]+pd[2]/2, y, nextXu, nextYu, book);
                       printf("329test--------pos[0]=%d, pos[1]=%d\n", pos[0], pos[1]);


                       double hi = sqrt(pow( pos[0]-pre_pos0,2)+pow(pos[1]-pre_pos1, 2));
//                               pixel4XY[0] = (sqrt(pow(sumx, 2)/(pow(k_horizontal,2)+1))/7)*abs(k_horizontal)*8;

                       double six_high =sqrt(pow(hi, 2)-pow(pd[2], 2));

                       double pre_pixel_hi = six_high/30 ;
//                       printf("329 pre_pixel_hi=%d, pre_pixel_hi*4=%d\n", pre_pixel_hi, pre_pixel_hi*4);
                    printf("329 hi=%f, six_high=%f, pre_pixel_hi=%f, pre_pixel_hi*4=%f\n", hi, six_high, pre_pixel_hi, pre_pixel_hi*4);
                        double temp = pre_pixel_hi*4;
//                        int Sum=0;
//                        for(int i=0; i<8;i++)
//                        {
//                            Sum += temp;
...
//                        }
           ...
           //            printf("va=%d\n", va);
                       ...
                       key = 1;//打开key递归开关
                       memset(book, 0, sizeof(book));
                       int nextXu2[4] = {-1, -1, -1,-2};
                       int nextYu2[4] = {0, 1, -1, 0};//向左找
                       pos[0]=0;
                       pos[1]=0;
                       key=1;
                       flood_fill_seed2(pic, ROW, ay[1]+ay_width[2]/2, nextXu2, nextYu2, book);
                       printf("329test--------pos[0]=%d, pos[1]=%d\n", pos[0], pos[1]);

                       double hi2 = sqrt(pow(pos[0]-pre_pos02,2)+pow(pos[1]-pre_pos12, 2));

                       double six_high2 =sqrt(pow(hi2, 2)-pow(ay_width[2], 2));

                       double pre_pixel_hi2 = six_high2/38 ;
                       double temp2 = pre_pixel_hi2*4;
                       printf("329 hi2=%f, six_high2=%f, pre_pixel_hi2=%f, pre_pixel_hi2*4=%f\n", hi2, six_high2, pre_pixel_hi2, pre_pixel_hi2*4);
...
void main()
{
    ...
    {
        printf("\n-------start------\n");
        char buffer[32]="picture_319\\frame+";//picture_319  new_picture  decode_picture quick_catch
        char cacahe[6];
        fra += 1;
